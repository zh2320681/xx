...
public class GetCaseQuestionsTask extends BaseAsyncTask {

	public GetCaseQuestionsTask(Context ctx, String title, String content,
			byte type) {
	...
	void afterTask(Result result) {
		...
		if(result.getSuccess()!=null && result.getSuccess().toLowerCase().equals("true")){
			if(mContext != null &&
					mContext instanceof CaseStudyActivity){
				CaseStudyActivity mCaseStudyActivity = (CaseStudyActivity)mContext;
				mCaseStudyActivity.afterTaskDoing(JSON.parseArray(result.getData(), CaseQuestion.class));
			}
			
			if(mContext != null &&
					mContext instanceof AnguiStudyActivity){
				AnguiStudyActivity mAnguiStudyActivity = (AnguiStudyActivity)mContext;
				mAnguiStudyActivity.afterTaskDoing(JSON.parseArray(result.getData(), CaseQuestion.class));
			}
//			finish();
		}else{
